...
TraceAnnotation = collections.namedtuple(
    "TraceKeyValueAnnotation", ["key", "value", "time"]
)
AggregatedReport = collections.namedtuple(
    "AggregatedReport", ["aggregated_values", "root_timer_data"]
)
...
class TimerData(object):
    ...
    def __repr__(self):
        return (
            "name=%r, span_id=%r start_time=%r end_time=%r annotations=%r, parent_span_id=%r,"
            "log_name=%r"
        ) % (
            self.name,
            self.span_id,
...
class StopWatch(object):
    ...
    def start(self, name, start_time=None):
        ...
        self._timer_stack.append(
            TimerData(
                ...
                parent_name=self._timer_stack[-1].log_name
                if self._timer_stack
                else None,
            )
        )
    ...
    def end(self, name, end_time=None, bucket=None):
        ...
        if self._should_trace_timer(log_name, tr_delta_ms):
            tr_data.parent_span_id = (
                self._timer_stack[-1].span_id if self._timer_stack else None
            )
        ...
        if not self._timer_stack:
            ...
            self._export_aggregated_timers_and_tracing_func(
                aggregated_report=agg_report, reported_traces=self._reported_traces
            )
    ...
    def _pop_stack(self, name):
        ...
        assert (not self._strict_assert) or (tr_data.name == name), (
            "StopWatch %s: %s, does not match latest start: %s"
            % (end_type, name, tr_data.name)
        )
    ...
    def _should_trace_timer(self, log_name, delta_ms):
        ...
        return bool(
            ...
            or self._reported_values[log_name][1]
            <= self.MAX_REQUEST_TRACING_SPANS_FOR_PATH
        )
