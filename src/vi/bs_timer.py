...
class TestPage(QtWebEngineWidgets.QWebEngineView):
    def __init__(self, contentFile:str, parent=None):
    ...
    def loadPage(self):
        if self.size().isNull():
            self.resize(640, 480)
        bUrl = QtCore.QUrl.fromLocalFile(self.file)
        self.load(bUrl)
        self.show()
...
class AddTimer:
    def __init__(self, testFile=None):
        if not testFile:
            testFile = os.path.join(os.curdir,"delve.svg")
...
if __name__ == "__main__":
    ...
    received = AddTimer().do_it()

    if received:
        page = TestPage(received)
        page.loadPage()
        app.exec()
